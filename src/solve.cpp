...
static bool process_command_line(
    ...
    auto print_soft_name = []() {
        fmt::print("GECOT — Graph-based Ecological Connectivity "
  ...
  gecot_solve <algorithm> <instance> <budget> [<params> ...]
    ...
    desc.add_options()("help,h", "Display this help message")(
        ...
        "verbose,v", "Enable all logging informations")(
    ...
    if(vm.count("algorithm")) {
        ...
        if(!found) {
            print_available_algorithms();
            throw std::logic_error(std::string("Error: the argument ('") + solver_name + "') for option '--algorithm' is invalid");
        }
    }
    ...
    if(vm.count("output-json")) {
        opt_output_json_file.emplace(vm["output-json"].as<std::filesystem::path>());
    }
    if(vm.count("output-csv")) {
        opt_output_csv_file.emplace(vm["output-csv"].as<std::filesystem::path>());
    }
    if(vm.count("quiet")) {
...
int main(int argc, const char * argv[]) {
    ...
    try {
        ...
        if(!process_command_line(args, solver, instances_description_json, budget,
                                opt_output_json_file, opt_output_csv_file)) return EXIT_SUCCESS;
        program_state = "Parsing instance";
        Instance raw_instance = parse_instance(instances_description_json);
        spdlog::info("Parsed instance: {}", std::filesystem::absolute(instances_description_json).string());
        ...
        const auto computation_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(sw.elapsed()).count();
        spdlog::info("Algorithm '{}' ran in {} ms", solver->name(), computation_time_ms);
        ...
        if(opt_output_json_file.has_value()){
            ...
            out.print("{{\n    \"initial_score\":  {},\n    \"budget\":         {},\n    \"solution_score\": {},\n    \"solution_cost\":  {},\n    \"solution\": {{", initial_score, budget, solution_score, solution_cost);
            const std::size_t option_name_max_length =
                std::ranges::max(std::ranges::views::transform(
                    raw_instance.options(),
                    [&](auto && o) { return raw_instance.option_name(o).size(); })) + 3;
            bool first_line = true;
            for(auto && option : raw_instance.options()) {
                auto option_name = std::string("\"") + raw_instance.option_name(option) + "\":";
                bool value = instance.contains_option(option_name)
                                ? solution[instance.option_from_name(option_name)]
                                : false;
                out.print("{}\n        {:<{}} {}", first_line ? "" : ",", option_name, option_name_max_length, value);
                first_line = false;
            }
            out.print("\n    }},\n    \"computation_time_ms\": {}\n}}", computation_time_ms);
        }
        ...
        if(opt_output_csv_file.has_value()){
            auto out = fmt::output_file(opt_output_csv_file.value().string().c_str());
            out.print("option_id,value\n");
            for(auto && option : raw_instance.options()) {
                auto option_name = raw_instance.option_name(option);
                int value =
                    static_cast<int>(instance.contains_option(option_name)
                        ? solution[instance.option_from_name(option_name)]
                        : false);
                out.print("\"{}\",{}\n", option_name, value);
            }
            spdlog::info("Solution written to '{}'", std::filesystem::absolute(opt_output_csv_file.value()).string());
        }
