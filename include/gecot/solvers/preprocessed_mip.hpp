...
// #include
// "gecot/preprocessing/compute_constrained_strong_and_useless_arcs.hpp"
...
//             for(auto && e : f.values) vars.emplace_back(std::visit(*this,
//             e));
...
//                 const auto [graph, quality_map, vertex_options_map,
//                 arc_no_map,
...
//                     F_vars(original_t)); model.add_constraint(F_prime_t_var
//                     <=
//                                          big_M_map[t].value() *
//                                          X_vars(option));
    ...
    instance_solution_t<I> solve(const I & instance,
        ...
        for(auto && instance_case : instance.cases()) {
            ...
            for(const auto & original_t : melon::vertices(original_graph)) {
                if(original_quality_map[original_t] == 0 &&
                   instance_case.vertex_options_map()[original_t].empty()) {
                    model.add_constraint(Xi_vars(original_t) <= 0);
                    continue;
                }
        ...
        auto get_vars_set = [&](const mip::constraint_id_t & begin_constraint,
                                const mip::constraint_id_t & end_constraint) {
            std::unordered_set<mip::variable_id_t> vars_set;
            for(auto && v : ranges::join_view(ranges::transform_view(
                    ranges::iota_view<mip::constraint_id_t,
                                      mip::constraint_id_t>(begin_constraint,
                                                            end_constraint),
                    [&model](auto && i) {
                        return ranges::views::transform(
                            model.constraint(i).expression().terms(),
        ...
        std::cout << "blocks " << (block_first_constraint.size() - 1)
                  << "\n\nmastervar";
        for(auto && v : get_vars_set(0, block_first_constraint[0]))
            std::cout << ' ' << model.column_name()[v].value();

        for(std::size_t i = 1; i < block_first_constraint.size(); ++i) {
            // std::cout << "\n\nblock " << i;
            // std::cout << "\n\tvar";
            // for(auto && v : get_vars_set(block_first_constraint[i - 1],
        ...
        //         for(auto && ic : instance.cases()) std::cout
        //     << " " << C_vars.var_name(ic.id());
        // for(auto && i : instance.options())
        //     std::cout << " " << X_vars.var_name(i);
        // std::cout << std::endl;
