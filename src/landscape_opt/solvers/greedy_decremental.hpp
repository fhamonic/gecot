...
struct GreedyDecremental {
    ...
    typename I::Solution solve(const I & instance, const double budget) const {
        ...
        while(purchaised > B) {
            ...
            options.erase(
                std::find(options.begin(), options.end(), worst_option));
            ...
            if(log_level > 1) {
                ...
                if(log_level > 2) {
                    for(auto const & [u, quality_gain] :
                        nodeOptions[worst_option])
        ...
        for(;;) {
            // std::cout << "il reste des options: " << free_options.size() <<
            // std::endl;

            // for(auto option : free_options) {
            //     std::cout << "option: " << option
            //               << "\tcost:" << purchaised + plan.getCost(option)
            //               <<
            //               "\tbudget:" << B
            //               << std::endl;
            // }
            ...
            free_options.erase(
                std::remove_if(
                    ...
                    [&](Option i) { return purchaised + plan.getCost(i) > B; }),
                free_options.end());

            // std::cout << "il en reste : " << free_options.size() <<
            // std::endl;
            ...
            free_options.erase(std::find(free_options.begin(),
                                         free_options.end(), best_option));
        ...
        if(log_level >= 1) {
            std::cout << name()
                      ...
                      << " ms" << std::endl;
            std::cout << name() << ": ECA from obj : " << solution.obj
                      << std::endl;
        }
