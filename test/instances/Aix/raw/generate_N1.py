...
for hexagon in N1_hexagons_csv:
    # if cost_to_prob(int(hexagon["cost"]), int(hexagon["import_rou"])) == 0:
    #     continue
...
N2_hexagons_csv = open("data/new_N2.csv", "w")
N2_hexagons_csv.write( 
    "id,N2_id,X,Y,area1,area2,area3,area4,cost,init_quality,id_troncon\n"
)
...
for id, hexagons in N2_hexagons.items():
    ...
    init_prob = product(
        [
            cost_to_prob(int(row["cost"]), int(row["import_rou"])) if row["cost"] != 1000 else 1
            for row in hexagons
        ]
    )
