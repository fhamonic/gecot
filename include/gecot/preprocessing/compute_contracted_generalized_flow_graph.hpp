#ifndef GECOT_COMPUTE_CONTRACTED_GENERALIZED_FLOW_GRAPH_HPP
#define GECOT_COMPUTE_CONTRACTED_GENERALIZED_FLOW_GRAPH_HPP
...
auto compute_contracted_generalized_flow_graph(const C & instance_case,
    ...
    for(const auto & original_uv : strong_arcs) {
        if(!instance_case.arc_options_map()[original_uv].empty())
